...
// actually I can use mono stack for even faster!
impl Solution {
    ...
    pub fn find_recur(start: usize, k: usize, remaining: usize, sums: &Vec<i32>, hashes: &mut Vec<HashMap<usize, Vec<usize>>>) -> Vec<usize> {
        ...
        while i <  limit {
            ...
            let tail_sum = ret.iter().map(|&i| sums[i]).sum::<i32>();
            let mut current = sums[i];
            let next_i = if !ret.is_empty() {
                ...
                std::cmp::max(ret[0] - k, i + 1)
            } else {
                i + 1
            };
            if  current + tail_sum > max {
                max = current + tail_sum;
                ret.insert(0, i);
                max_vec = ret;
            }
            i = next_i;
...
fn test() {
    for i in vec![
        ...
        (vec![18,11,14,7,16,4,18,11,4,8], 2, vec![0, 3, 6]),
    ] {
